...
masterdir = 'C:\Users\phili\Dropbox (Cognitive and Affective Neuroscience Laboratory)\';
...
figsavedir=strrep(figsavedir,'figures','figures_for_manuscript_forGenTest');
dosurfaces = 0;

%% Load master data and pull out subjects we want for this paper

[behdat] = import_data_extract_info;
% isControl=strcmp('Control',{behdat.textdata.Data{2:end,4}});
isControl=strcmp('Control',behdat.patientgroup);

disp(behdat.newnames)

%wh_groups = 1:5; % all but vaginal (this includes patients)
%wh_groups = [1 2 4 8]; % all but vaginal (this includes Controls ONLY)
% wh_groups = [1 2 4 6]; % all but vaginal (this includes Controls ONLY)
wh_groups = [1 2 4 6 8 9 10]; % all (this includes Controls ONLY)
